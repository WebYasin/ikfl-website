...
const create = async (req, res, next) => {
    ...
    try {
        ...
        if (category.files.length) category.files = category.files.map(file => file._id);
...
const get = async (req, res, next) => {
    try {
        ...
        let docs = await CategoryModel.find(query).sort({createdAt: -1}).limit(limit).skip(pagination*limit).populate('files', 'name original path thumbnail smallFile');
